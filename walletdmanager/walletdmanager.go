...
var (
	...
	// will be set to a random string when starting lithe-service
	rpcPassword = ""

	cmdWalletd     *exec.Cmd
	cmdLithed *exec.Cmd

	// WalletdOpenAndRunning is true when lithe-service is running with a wallet open
...
func RequestBalance() (availableBalance float64, lockedBalance float64, totalBalance float64, err error) {
	...
	availableBalance, lockedBalance, totalBalance, err = lithewalletdrpcgo.RequestBalance(rpcPassword)
	if err != nil {
...
func RequestAvailableBalanceToBeSpent(transferFeeString string) (availableBalance float64, err error) {
	...
	transferFee, err := strconv.ParseFloat(transferFeeString, 64) // transferFee is expressed in LITHE
	if err != nil {
...
func RequestAddress() (address string, err error) {
	...
	address, err = lithewalletdrpcgo.RequestAddress(rpcPassword)
	if err != nil {
...
func RequestListTransactions() (transfers []lithewalletdrpcgo.Transfer, err error) {

	walletBlockCount, _, _, _, err := lithewalletdrpcgo.RequestStatus(rpcPassword)
	if err != nil {
		log.Error("error getting block count: ", err)
		return nil, err
	}

	transfers, err = lithewalletdrpcgo.RequestListTransactions(walletBlockCount, 1, []string{WalletAddress}, rpcPassword)
	if err != nil {
...
func SendTransaction(transferAddress string, transferAmountString string, transferPaymentID string, transferFeeString string) (transactionHash string, err error) {
	...
	if !strings.HasPrefix(transferAddress, "ethiL") || (len(transferAddress) != 99 && len(transferAddress) != 186) {
	...
	transferAmount, err := strconv.ParseFloat(transferAmountString, 64) // transferAmount is expressed in LITHE
	if err != nil {
		return "", errors.New("amount is invalid")
	}

	if transferAmount <= 0 {
		return "", errors.New("amount of LITHE to be sent should be greater than 0")
	}
	...
	transactionHash, err = lithewalletdrpcgo.SendTransaction(transferAddress, transferAmount, transferPaymentID, transferFee, rpcPassword)
	if err != nil {
...
func OptimizeWalletWithFusion() (transactionHash string, err error) {
	...
	transactionHash, err = lithewalletdrpcgo.SendFusionTransaction(smallestOptimizedThreshold, []string{WalletAddress}, WalletAddress, rpcPassword)
	if err != nil {
...
func getOptimisedFusionParameters() (largestFusionReadyCount int, smallestOptimizedThreshold int, err error) {
	...
	for {
		fusionReadyCount, _, err := lithewalletdrpcgo.EstimateFusion(threshold, []string{WalletAddress}, rpcPassword)
		if err != nil {
...
func GetPrivateKeys() (isDeterministicWallet bool, mnemonicSeed string, privateViewKey string, privateSpendKey string, err error) {
	...
	isDeterministicWallet, mnemonicSeed, err = lithewalletdrpcgo.GetMnemonicSeed(WalletAddress, rpcPassword)
	if err != nil {
		log.Error("error requesting mnemonic seed. err: ", err)
		return false, "", "", "", err
	}

	privateViewKey, err = lithewalletdrpcgo.GetViewKey(rpcPassword)
	if err != nil {
		log.Error("error requesting view key. err: ", err)
		return false, "", "", "", err
	}

	privateSpendKey, _, err = lithewalletdrpcgo.GetSpendKeys(WalletAddress, rpcPassword)
	if err != nil {
...
// SaveWallet saves the sync status of the wallet. To be done regularly so when lithe-service crashes, sync is not lost
func SaveWallet() (err error) {

	err = lithewalletdrpcgo.SaveWallet(rpcPassword)
	if err != nil {
...
// StartWalletd starts the lithe-service daemon with the set wallet info
...
// useCheckpoints is true if Lithed should be run with "--load-checkpoints"
func StartWalletd(walletPath string, walletPassword string, useRemoteNode bool, useCheckpoints bool, daemonAddress string, daemonPort string) (err error) {
	...
	if isWalletdRunning() {
		errorMessage := "lithe-service is already running in the background.\nPlease close it via "
	...
	pathToLogLithedCurrentSession := logsFolder + directorySeparator + logLithedCurrentSessionFilename
	pathToLogLithedAllSessions := logsFolder + directorySeparator + logLithedAllSessionsFilename

	pathToWalletd := "./" + walletdCommandName
	pathToLithed := "./" + lithedCommandName
	...
	if isPlatformDarwin {
		...
		pathToLithed = pathToAppContents + "/" + lithedCommandName
		...
		pathToAppLibDir := pathToHomeDir + "/Library/Application Support/Lithe-Nest"
		...
		pathToLogLithedCurrentSession = pathToAppLibDir + "/" + pathToLogLithedCurrentSession
		pathToLogLithedAllSessions = pathToAppLibDir + "/" + pathToLogLithedAllSessions
		...
		pathToLithed = pathToAppDirectory + "/" + lithedCommandName
		...
		pathToLogLithedCurrentSession = pathToAppDirectory + "/" + pathToLogLithedCurrentSession
		pathToLogLithedAllSessions = pathToAppDirectory + "/" + pathToLogLithedAllSessions
		if pathToWallet == WalletFilename {
			// if comes from createWallet, so it is not a full path, just a filename
	...
	var lithedCurrentSessionLogFile *os.File
	...
	if !useRemoteNode && !isLithedRunning() {

		lithedCurrentSessionLogFile, err = os.Create(pathToLogLithedCurrentSession)
		if err != nil {
			log.Error(err)
		}
		defer lithedCurrentSessionLogFile.Close()

		if useCheckpoints {
			cmdLithed = exec.Command(pathToLithed, "--load-checkpoints", pathToCheckpointsCSV, "--log-file", pathToLogLithedCurrentSession)
		} else {
			cmdLithed = exec.Command(pathToLithed, "--log-file", pathToLogLithedCurrentSession)
		}
		hideCmdWindowIfNeeded(cmdLithed)

		lithedAllSessionsLogFile, err := os.OpenFile(pathToLogLithedAllSessions, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600)
		if err != nil {
			log.Error(err)
		}
		cmdLithed.Stdout = lithedAllSessionsLogFile
		defer lithedAllSessionsLogFile.Close()
		...
		log.Info("Opening Lithed and waiting for it to be ready.")

		readerLithedLog := bufio.NewReader(lithedCurrentSessionLogFile)

		for {
			line, err := readerLithedLog.ReadString('\n')
			if err != nil {
				if err != io.EOF {
					log.Error("Failed reading Lithed log file line by line: ", err)
				}
			}
			if strings.Contains(line, "Imported block with index") {
				log.Info("Lithed importing blocks: ", line)
			}
			if strings.Contains(line, "Core rpc server started ok") {
				log.Info("Lithed ready (rpc server started ok).")
				break
			}
			if strings.Contains(line, "Node stopped.") {
				errorMessage := "Error Lithed: 'Node stopped'"
	...
	log.Info("Opening lithe-service and waiting for it to be ready.")
	...
	for !successLaunchingWalletd && time.Duration(timesCheckLog)*timeBetweenChecks < maxWaitingTime {
		...
		for {
			...
			if strings.Contains(line, "Wallet loading is finished.") {
				...
				log.Info("lithe-service ready ('Wallet loading is finished.').")
				break
			}
			if strings.Contains(line, "Imported block with index") {
				timesCheckLog = 0
			...
			if strings.Contains(line, "INFO    Stopped") {
				...
				if len(listWalletdErrors) > 0 {
					...
					errorMessage = "lithe-service stopped with unknown error"
				}
	...
	_, _, _, _, err = lithewalletdrpcgo.RequestStatus(rpcPassword)
	if err != nil {
		killWalletd()
		return errors.New("error communicating with lithe-service via rpc")
	}
...
func GracefullyQuitWalletd() {
	...
	if WalletdOpenAndRunning || cmdWalletd != nil {
		...
		if isPlatformWindows {
			...
			lithewalletdrpcgo.SaveWallet(rpcPassword)
			time.Sleep(3 * time.Second)

			err = cmdWalletd.Process.Kill()
			if err != nil {
				log.Error("failed to kill lithe-service: " + err.Error())
			} else {
				log.Info("lithe-service killed without error")
			}
		} else {
			...
			case <-time.After(5 * time.Second):
				if err := cmdWalletd.Process.Kill(); err != nil {
					log.Warning("failed to kill lithe-service: " + err.Error())
				}
				log.Info("lithe-service killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("lithe-service finished with error: " + err.Error())
				}
				log.Info("lithe-service killed without error")
			}
		}
...
// GracefullyQuitLithed stops the Lithed daemon
func GracefullyQuitLithed() {

	if cmdLithed != nil {
		var err error

		if isPlatformWindows {
			// because syscall.SIGTERM does not work in windows. We have to kill Lithed.

			err = cmdLithed.Process.Kill()
			if err != nil {
				log.Error("failed to kill Lithed: " + err.Error())
			} else {
				log.Info("Lithed killed without error")
			}
		} else {
			_ = cmdLithed.Process.Signal(syscall.SIGTERM)
			done := make(chan error, 1)
			go func() {
				done <- cmdLithed.Wait()
			}()
			select {
			case <-time.After(5 * time.Second):
				if err := cmdLithed.Process.Kill(); err != nil {
					log.Warning("failed to kill Lithed: " + err.Error())
				}
				log.Info("Lithed killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("Lithed finished with error: " + err.Error())
				}
				log.Info("Lithed killed without error")
			}
		}
...
// CreateWallet calls lithe-service to create a new wallet. If privateViewKey, privateSpendKey and mnemonicSeed are empty strings, a new wallet will be generated. If they are not empty, a wallet will be generated from those keys or from the seed (import)
// walletFilename is the filename chosen by the user. The created wallet file will be located in the same folder as lithe-service.
...
func CreateWallet(walletFilename string, walletPassword string, walletPasswordConfirmation string, privateViewKey string, privateSpendKey string, mnemonicSeed string, scanHeight string) (err error) {
	...
	if WalletdOpenAndRunning {
		return errors.New("lithe-service is already running. It should be stopped before being able to generate a new wallet")
	}
...
func RequestConnectionInfo() (syncing string, walletBlockCount int, knownBlockCount int, localDaemonBlockCount int, peerCount int, err error) {
	...
	walletBlockCount, knownBlockCount, localDaemonBlockCount, peerCount, err = lithewalletdrpcgo.RequestStatus(rpcPassword)
	if err != nil {
		return "", 0, 0, 0, 0, err
	}
...
func RequestFeeinfo() (nodeFee float64, err error) {
	...
	_, nodeFee, _, err = lithewalletdrpcgo.GetFeeInfo(rpcPassword)
	if err != nil {
		return 0, err
	}
...
func isLithedRunning() bool {

	if _, _, err := findProcess(lithedCommandName); err == nil {
		return true
	}

	if isPlatformWindows {
		if _, _, err := findProcess(lithedCommandName + ".exe"); err == nil {
			return true
		}
	}
