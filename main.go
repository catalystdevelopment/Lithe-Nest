...
var (
	transfers                   []lithewalletdrpcgo.Transfer
	remoteNodes                 []node
	...
	stringBackupKeys            = ""
	rateUSDLITHE                float64 // USD value for 1 LITHE
...
func main() {
	...
	if isPlatformDarwin {
		...
		pathToAppFolder := pathToHomeDir + "/Library/Application Support/Lithe-Nest"
	...
	walletdmanager.GracefullyQuitLithed()
}
...
func getAndDisplayBalances() {
	...
	if err == nil {
		...
		balanceUSD := walletTotalBalance * rateUSDLITHE
	...
	if err == nil {
		...
		if needFullUpdate {
			...
			for index, transfer := range transfers {
				...
				amountString += " LITHE (fee: " + strconv.FormatFloat(transfer.Fee, 'f', 2, 64) + ")"
...
func transfer(transferAddress string, transferAmount string, transferPaymentID string, transferFee string) {
	...
	qmlBridge.FinishedSendingTransaction()
	qmlBridge.DisplayPopup("LITHEs sent successfully", 4000)
}
...
func startWalletWithWalletInfo(pathToWallet string, passwordWallet string) bool {
	...
	if err != nil {
		log.Warn("error starting lithe-service with provided wallet info. error: ", err)
		qmlBridge.FinishedLoadingWalletd()
		qmlBridge.DisplayErrorDialog("Error opening wallet.", err.Error())
		return false
	}

	log.Info("success starting lithe-service")
...
func getFullBalanceAndDisplayInTransferAmount(transferFee string) {
	...
	qmlBridge.DisplayFullBalanceInTransferAmount(humanize.FtoaWithDigits(availableBalance, 5))
}

func getDefaultFeeAndDisplay() {

	qmlBridge.DisplayDefaultFee(humanize.FtoaWithDigits(walletdmanager.DefaultTransferFee, 5))
}
...
func getNodeFeeAndDisplay() {
	...
	if err != nil {
		...
		qmlBridge.DisplayNodeFee(humanize.FtoaWithDigits(nodeFee, 5))
	}
}
...
func requestRateLITHE() {
	response, err := http.Get(urlCryptoCompareLITHE)
	...
	if err != nil {
		...
		if err != nil {
			...
			if err := json.Unmarshal(b, &resultInterface); err != nil {
				...
				rateUSDLITHE = resultsMap["USD"].(float64)
			}
		}
	}
}
...
func amountStringUSDToLITHE(amountLITHEString string) string {
	amountLITHE, err := strconv.ParseFloat(amountLITHEString, 64)
	if err != nil || amountLITHE <= 0 || rateUSDLITHE == 0 {
		return ""
	}
	amountUSD := amountLITHE * rateUSDLITHE
