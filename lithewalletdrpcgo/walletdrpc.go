...
// Package lithewalletdrpcgo handles the the rpc connection between your app and ithe-service
package lithewalletdrpcgo
...
var (
	rpcURL = "http://127.0.0.1:25502/json_rpc"
)

// RequestBalance provides the available and locked balances of the current wallet
// returned balances are expressed in LITHE, not in 0.00010 LITHE
func RequestBalance(rpcPassword string) (availableBalance float64, lockedBalance float64, totalBalance float64, err error) {
	...
	availableBalance = responseMap["result"].(map[string]interface{})["availableBalance"].(float64) / 100000
	lockedBalance = responseMap["result"].(map[string]interface{})["lockedAmount"].(float64) / 100000
...
func RequestListTransactions(blockCount int, firstBlockIndex int, addresses []string, rpcPassword string) (transfers []Transfer, err error) {
	...
	for _, block := range blocks {
		...
		for _, transaction := range transactions {
			...
			transfer.Amount = mapTransaction["amount"].(float64) / 100000
			transfer.Fee = mapTransaction["fee"].(float64) / 100000
...
// RequestStatus requests lithe-service connection and sync status
...
// parameters amount and fee are expressed in LITHE, not 0.00010 LITHE
func SendTransaction(addressRecipient string, amount float64, paymentID string, fee float64, rpcPassword string) (transactionHash string, err error) {
	...
	amountInt := uint64(amount * 100000) // expressed in 100k of LITHE
	feeInt := uint64(fee * 100000)       // expressed in 100k of LITHE
...
// threshold is the value that determines which outputs will be optimized. Only the outputs, lesser than the threshold value, will be included into a fusion transaction (threshold is expressed in LITHE, not 0.00010 LITHE).
...
func EstimateFusion(threshold int, addresses []string, rpcPassword string) (fusionReadyCount int, totalOutputCount int, err error) {
	...
	threshold *= 100000 // expressed in 100k of LITHE
...
// returned fee is expressed in LITHE, not in 0.00010 LITHE
func GetFeeInfo(rpcPassword string) (address string, fee float64, status string, err error) {
	...
	if resultAmount != nil {
		fee = resultAmount.(float64) / 100000
	} else {
		fee = 0
	}
